...
class SinglePassEngine(Engine):
    def __init__(self, model: Model, system_prompt: Optional[str] = None, session_id: Optional[str] = None):
        self.model = model
    ...
    async def execute(self, prompt: str) -> str:
        ...
        await self.bus.publish(SinglePassEngineStatusEvent(status="Calling LLM", session_id=self.session_id))
        response = await self.model.generate(context)
        await self.bus.publish(SinglePassEngineStatusEvent(status="finished", session_id=self.session_id))
        return response.content

async def use_single_pass_engine(prompt: str, model: Model, system_prompt: Optional[str] = None):
...
async def main(case: int):
    ...
    if case == 1:
        engine = SinglePassEngine(Gpt41Mini(Providers.OPENAI), "respond in pirate", "test")
        cli = EngineCLI("test")
        ...
        result = await use_single_pass_engine("Hello, world!", Gpt41Mini(Providers.OPENAI), "respond in pirate")
        print(result)
