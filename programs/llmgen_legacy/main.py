...
from programs.llmgen_legacy.chat import LLMGenChat, LLMGenConfig
from framework.types.engine_types import EngineTypeMap
...
app = typer.Typer(
    help="LLMGen Chat - Advanced LLM-powered chat interface", add_completion=False
)
...
def chat(
    ...
    model: str = typer.Option("gpt-4o-mini", "--model", help="Model to use for chat"),
    ...
    history_file: Optional[Path] = typer.Option(
        None, "--history", "-h", help="Path to load chat history from"
    ),
    streaming: Optional[bool] = typer.Option(False, help="Enable streaming mode"),
):
    ...
    try:
        ...
        config = LLMGenConfig(
            model_request_config=ModelRequestConfig(model_name=model),
