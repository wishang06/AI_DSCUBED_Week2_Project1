...
#             self._create_completion = self.client.create_completion_legacy
#
#     def subscribe(self, observer: Observer):
#         self.subject.register(observer)
#
...
#             }
#         )
#         response = self._create_completion(self.model, self.store.retrieve())
...
#         else:
#             self.streaming = False
#             self.create_completion = self.client.create_completion_legacy
#
#     def toggle_streaming(self):
#         if self.streaming:
#             self.create_completion = self.client.create_completion_legacy
...
#         )
#         self.subject.notify({"type": "response", "content": response.full})
...
#         else:
#             self.store.store_response(response, "assistant")
#         self.subject.notify({"type": "status_update", "message": "done"})
#         return response
#
...
#         self.subject.notify(
#             {"type": "status_update", "message": "Getting LLM Response..."}
#         )
#         self.store.store_string(prompt, "user")
#         response = self.client.create_tool_completion(
#             self.model_name,
...
#             "These are the tools you have:" + str(create_tools_schema(self.tools)),
#             "assistant",
#         )
#         with self.callback as loading:
#             self.store.store_string(
...
#                         response, create_tools_lookup(self.agent_tools), loading
#                     )
#                 else:
#                     self.store.store_response(response, "assistant")
#             self.store.store_string(
...
#                 "assistant",
#             )
#             response = self.client.create_completion_legacy_v2(
#                 self.model_name, self.store.retrieve()
#             )
#             self.store.store_response(response, "assistant")
...
#                 )
#                 self.process_tool_calls(
#                     response, create_tools_lookup(self.tools), loading
#                 )
...
#                 content=str(result)[0:500], author="Tool Result", style="yellow"
#             )
...
#                             }
#                         )
#                         plan_response = self.client.create_completion_legacy_v2(
...
#             if self.temp != "":
#                 self.callback.print_message(
#                     content=self.temp, author="Assistant", style="green"
#                 )
...
#         response = self.client.create_completion_legacy_v2(self.model_name, context)
#         self.store.store_response(response, "assistant")
#         return response
