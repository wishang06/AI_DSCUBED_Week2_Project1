...
class ObservabilityBus:
    ...
    def _setup_default_handlers(self, log_dir: str) -> None:
        ...
        logger.info(f"Observability bus initialized with handlers: {[h.__class__.__name__ for h in self._handlers]}")
    ...
    def add_handler(self, handler: Any) -> None:
        ...
        if not isinstance(handler, ObservabilityHandler):
            raise TypeError(f"Handler must be an ObservabilityHandler, got {type(handler)}")
    ...
    def metric(self, name: str, value: Union[int, float], unit: Optional[str] = None, 
